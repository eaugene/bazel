...
public final class IntegrationTestUtils {
  ...
  public static WorkerInstance createWorker(boolean useHttp) {
    return new WorkerInstance(
        useHttp, newWorkerTmpDir(), /* failureCount= */ 0, /* failureMethod= */ "");
  }
  ...
  public static class WorkerInstance implements TestRule {
    ...
    private WorkerInstance(boolean useHttp, Path dir, int failureCount, String failureMethod) {
    ...
    private void start() throws IOException, InterruptedException {
      ...
      ImmutableList.Builder<String> argv =
          ImmutableList.<String>builder()
              .add(workerPath)
              .add("--work_path=" + workPath)
              .add("--cas_path=" + casPath)
              .add((useHttp ? "--http_listen_port=" : "--listen_port=") + port);
      if (failureCount > 0) {
